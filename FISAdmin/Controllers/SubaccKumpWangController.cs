...
{
    public class SubaccKumpWangController : Controller
    {
        private readonly IConfiguration config;
        public List<SubaccModel> result = new List<SubaccModel>();
        public string type = "Subaccount Kumpulan Wang";


        public SubaccKumpWangController(IConfiguration configuration)
        {
...
#pragma warning disable CS8602 // Dereference of a possibly null reference.
            if (obj.Kod.ToString() == obj.Tajuk.ToString())
            {
                ...
                TempData["error"] = "Subaccount Kumpulan Wang cannot be created";
            }
            else
            {
                ...
                using (SqlConnection con = new SqlConnection(config.GetConnectionString("dbfis")))
                {
                    string sql = "SELECT kod FROM subaccount_kump_wang WHERE kod=@kod";
                    SqlCommand cmd = new SqlCommand(sql, con);
                    ...
                    else
                    {
                        ...
                        using (SqlConnection con2 = new SqlConnection(config.GetConnectionString("dbfis")))
                        {
                            ...
                            string sql2 = "INSERT INTO subaccount_kump_wang (kod,tajuk,CreatedBy,CreatedDate,LastModifiedBy,LastModifiedDate) VALUES (@kod,@tajuk,@CreatedBy,@CreatedDate,@LastModifiedBy,@LastModifiedDate)";
                            ...
                            TempData["success"] = "Subaccount Kumpulan Wang created successfully";
        ...
        public IActionResult Edit(string id)
        {
            ...
            using (SqlConnection con = new SqlConnection(config.GetConnectionString("dbfis")))
            {
                string sql = "SELECT kod,tajuk,CreatedBy,CreatedDate,LastModifiedBy,LastModifiedDate FROM subaccount_kump_wang WHERE kod=@kod";
                SqlCommand cmd = new SqlCommand(sql, con);
        ...
        public IActionResult Edit()
        {
            ...
            else
            {
                using (SqlConnection con = new SqlConnection(config.GetConnectionString("dbfis")))
                {
                    string sql = "UPDATE subaccount_kump_wang SET tajuk=@tajuk,LastModifiedBy=@LastModifiedBy,LastModifiedDate=@LastModifiedDate WHERE kod=@kod";
                    ...
                    cmd.ExecuteNonQuery();
                    TempData["success"] = "Subaccount Kumpulan Wang updated successfully";
                    return RedirectToAction(nameof(Index));
        ...
        public IActionResult Delete()
        {
            using (SqlConnection con = new SqlConnection(config.GetConnectionString("dbfis")))
            {
                string sql = "DELETE FROM subaccount_kump_wang WHERE kod=@kod";
            ...
            TempData["success"] = "Subaccount Kumpulan Wang deleted successfully";
            ViewData["type"] = type;
